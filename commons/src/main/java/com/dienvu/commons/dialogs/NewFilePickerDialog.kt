...
import com.dienvu.commons.adapters.NewFilepickerFavoritesAdapter
import com.dienvu.commons.adapters.NewFilepickerItemsAdapter
import com.dienvu.commons.extensions.*
import com.dienvu.commons.helpers.ensureBackgroundThread
import com.dienvu.commons.models.FileDirItem
import com.dienvu.commons.views.Breadcrumbs
import kotlinx.android.synthetic.main.dialog_filepicker.view.*
import java.io.File

class NewFilePickerDialog(
    val activity: NewBaseSimpleActivity,
    ...
    :Breadcrumbs.BreadcrumbsListener
{
    ...
    init
    {
        ...
        mDialogView.filepicker_breadcrumbs.apply {
            listener = this@NewFilePickerDialog
    ...
    private fun getTitle() =
        if (pickFile) R.string.select_file
        else R.string.select_folder
    private fun createNewFolder()
    {
        NewCreateNewFolderDialog(activity, currPath)
        {
            callback(it)
            mDialog?.dismiss()
        }
    ...
    private fun updateItems(items: ArrayList<FileDirItem>)
    {
        ...
        val adapter = NewFilepickerItemsAdapter(activity, sortedItems, mDialogView.filepicker_list)
        {
    ...
    private fun getRegularItems(path: String, lastModifieds: HashMap<String, Long>, callback: (List<FileDirItem>) -> Unit)
    {
        ...
        for (file in files)
        {
            ...
            val children =
                if (isDirectory) file.getDirectChildrenCount(activity, showHidden)
                else 0
    ...
    override fun breadcrumbClicked(id: Int)
    {
        if (id == 0)
        {
            NewStoragePickerDialog(activity, currPath, forceShowRoot, true)
            {
                currPath = it
                tryUpdateItems()
            }
