...
import com.dienvu.commons.extensions.normalizePhoneNumber
import com.dienvu.commons.extensions.normalizeString
import com.dienvu.commons.helpers.SORT_BY_FULL_NAME
import com.dienvu.commons.helpers.SORT_DESCENDING
    ...
    : Comparable<SimpleContact>
{
    ...
    private fun compareByFullName(other: SimpleContact): Int
    {
        ...
        return if (
            firstString.firstOrNull()?.isLetter() == true &&
            secondString.firstOrNull()?.isLetter() == false)
        {
            -1
        }
        else if (
            firstString.firstOrNull()?.isLetter() == false &&
            secondString.firstOrNull()?.isLetter() == true)
        {
            1
        }
        else
    ...
    fun doesHavePhoneNumber(text: String): Boolean
    {
        return if (text.isNotEmpty())
        {
            ...
            {
                phoneNumbers.map { it.normalizedNumber }.any { phoneNumber ->
                    PhoneNumberUtils.compare(phoneNumber.normalizePhoneNumber(),
                        normalizedText) ||
                        phoneNumber == text ||
