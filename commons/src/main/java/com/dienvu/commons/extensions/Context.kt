...
import com.dienvu.commons.helpers.MyContentProvider.Companion.COL_ACCENT_COLOR
import com.dienvu.commons.helpers.MyContentProvider.Companion.COL_APP_ICON_COLOR
import com.dienvu.commons.helpers.MyContentProvider.Companion.COL_BACKGROUND_COLOR
import com.dienvu.commons.helpers.MyContentProvider.Companion.COL_LAST_UPDATED_TS
import com.dienvu.commons.helpers.MyContentProvider.Companion.COL_NAVIGATION_BAR_COLOR
import com.dienvu.commons.helpers.MyContentProvider.Companion.COL_PRIMARY_COLOR
import com.dienvu.commons.helpers.MyContentProvider.Companion.COL_TEXT_COLOR
import com.dienvu.commons.models.AlarmSound
import com.dienvu.commons.models.BlockedNumber
...
fun Context.getFilePublicUri(file: File, applicationId: String): Uri
{
    ...
    // if media content uri is null, get our custom uri like content://com.dienvu.gallery.provider/external_files/emulated/0/DCIM/IMG_20171104_233915.jpg
    var uri =
        if (file.isMediaFile())
        {
...
fun Context.getMyContactsCursor(favoritesOnly: Boolean, withPhoneNumbersOnly: Boolean) =
    try
    {
    val getFavoritesOnly =
        if (favoritesOnly) "1"
        else "0"
    val getWithPhoneNumbersOnly =
        if (withPhoneNumbersOnly) "1"
        else "0"
...
fun Context.getDialogTheme() =
    if (baseConfig.backgroundColor.getContrastColor() == Color.WHITE) R.style.MyDialogTheme_Dark
...
fun Context.isThankYouInstalled() = isPackageInstalled("com.dienvu.thankyou")
...
fun Context.isAProApp() = packageName.startsWith("com.dienvu.") && packageName.removeSuffix(".debug").endsWith(".pro")
...
fun Context.getCanAppBeUpgraded() = proPackages.contains(baseConfig.appId.removeSuffix(".debug").removePrefix("com.dienvu."))
fun Context.getProUrl() = "https://play.google.com/store/apps/details?id=${baseConfig.appId.removeSuffix(".debug")}.pro"
fun Context.getStoreUrl() ="https://play.google.com/store/apps/details?id=com.dienvu.timemanager.pro}"
...
fun Context.isDefaultDialer(): Boolean {
    return if (!packageName.startsWith("com.dienvu.contacts") && !packageName.startsWith("com.dienvu.dialer")) {
        true
    } else if ((packageName.startsWith("com.dienvu.contacts") || packageName.startsWith("com.dienvu.dialer")) && isQPlus()) {
...
fun Context.copyToClipboard(text: String) {
    val clip = ClipData.newPlainText(getString(R.string.dienvu_commons), text)
