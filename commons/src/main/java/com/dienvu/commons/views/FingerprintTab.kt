...
import com.dienvu.commons.helpers.PROTECTION_FINGERPRINT
...
class FingerprintTab(context: Context, attrs: AttributeSet) : RelativeLayout(context, attrs), SecurityTab {
    ...
    private val registerHandler = Looper.myLooper()?.let { Handler(it) }
        ...
        Reprint.authenticate(object : AuthenticationListener {
            ...
            override fun onFailure(failureReason: AuthenticationFailureReason, fatal: Boolean, errorMessage: CharSequence?, moduleTag: Int, errorCode: Int) {
                when (failureReason) {
                    ...
                    AuthenticationFailureReason.NO_HARDWARE -> context.toast("No Hardware")
                    AuthenticationFailureReason.HARDWARE_UNAVAILABLE -> context.toast("Hardware Unavailable")
                    AuthenticationFailureReason.NO_FINGERPRINTS_REGISTERED-> context.toast("No Fingerprints Registered")
                    AuthenticationFailureReason.SENSOR_FAILED -> context.toast("Sensor failed")
                    AuthenticationFailureReason.TIMEOUT-> context.toast("Time out")
                    AuthenticationFailureReason.UNKNOWN-> context.toast("Unknown")
                }
            }
        })
