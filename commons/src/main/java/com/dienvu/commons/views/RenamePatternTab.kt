...
import com.dienvu.commons.helpers.isNougatPlus
import com.dienvu.commons.interfaces.RenameTab
import com.dienvu.commons.models.Android30RenameFormat
...
class RenamePatternTab(context: Context, attrs: AttributeSet) : RelativeLayout(context, attrs), RenameTab
{
            ...
            activity?.checkManageMediaOrHandleSAFDialogSdk30(firstPath) {
                ...
                for (path in validPaths)
                {
                    ...
                    try
                    {
                        ...
                        activity?.renameFile(path, newPath, true)
                        { success, android30Format ->
                            if (success)
                            {
                                pathsCnt--
                                if (pathsCnt == 0)
    ...
    private fun getNewPath(path: String, useMediaFileExtension: Boolean): String?
    {
        try
        {
            ...
            val pattern =
                if (dateTime.substring(4, 5) == "-") "yyyy-MM-dd kk:mm:ss"
                else "yyyy:MM:dd kk:mm:ss"
